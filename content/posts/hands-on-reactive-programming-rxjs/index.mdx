...
> I have a stream **A**, and a stream **B**. If I want to subscribe to stream **B** once stream **A** emits a value, I need to pass the flow control from **A** to **B**, meaning we'll flatten our streams to be left with a single one. Instead of having **A** and **B** we'll be left with **B** only, and **A** will in a way be "left behind", we're interested in values pushed to **B** now. The subscription to **B** is our inner subscription. If it helps, whenever you read "flattening" think of a subscription inside subscription.

That's where the `switchMap` operator comes into picture. `switchMap` allows you to trigger a subscription from within a stream to a second stream and subscribe to its updates! Not clear yet? Think of it this way `switchMap` **allows you to switch to a new observable**. Ok, let's look at our countdown implementation, now that we have this information.

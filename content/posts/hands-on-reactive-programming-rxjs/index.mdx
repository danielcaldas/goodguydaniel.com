...
- [x] Compress all the images & GIFs in the article
...
- [ ] Each first time an operator appears it must link to the docs!!
- [ ] Reduce quality of GIFs... they are too big
...
Cool, we now have a stream that emits a `drop` event every time something is dropped in the `party`, we ensure that using the <a href="https://rxjs-dev.firebaseapp.com/api/operators/filter" target="_blank" title="RxJS - filter">filter</a> operator. I would like to pause and add further detail on this marble diagram. Notice that we read from top to bottom, meaning the last timeline is the only one that really matters, because in practice we'll just subscribe to `partyDrop$`. In this case since we won't subscribe to `drop$` directly, think of it as an auxiliary conceptual timeline which is a step towards shaping the ideal stream to tackle our task: `partyDrop$`.
...
catDragStartDisplay$.subscribe((v) => loading.style.setProperty("display", v));
catDragEndDisplay$.subscribe((v) => loading.style.setProperty("display", v));
```
...
<Caption
  text={() => (
    <p>
      <b>A single stream becomes the source of truth</b> for the display property value in the <b>loading</b> element
      through time.
    </p>
  )}
/>

Awesome! The <a href="https://rxjs-dev.firebaseapp.com/api/operators/merge" target="_blank" title="RxJS - merge">merge</a> operator gives us the power to take to streams and merge them into one so that the values of the `"display"` of the `loading` element flow through a single stream (a single source of truth). Here's how the code looks like.
...
At the time of this writing, I feel overwhelmed by the tremendous amount of operators offered by RxJS, its API is considerably large. We could further simplify the above implementation with the operator <a href="https://rxjs-dev.firebaseapp.com/api/operators/mapTo" target="_blank" title="RxJS - mapTo">mapTo</a>.
...
We could drop `catDragStartDisplay$` and `catDragEndDisplay$` by inlining them in the `merge` operator, anyways we're not subscribing to them.
...
And that's a rough implementation of a countdown. We need to keep track of the timer identifier in a variable to ensure we clean up after running the `setInterval` provided callback the expected amount of times.
...
Let's examine the above code 🤯. This time we're not creating a stream from an event or anything like that. Instead, we use <a href="https://rxjs-dev.firebaseapp.com/api/index/function/timer" target="_blank" title="RxJS - mapTo">timer</a> from RxJS to generate a timer stream that emits a new value every second.
...
2. **Less error-prone code** by relying on `takeWhile` built-in operator to limit the number of times our subscription runs.
...
<Caption
  text={() => (
    <p>
      Representation of our <b>countdown$</b> stream.
    </p>
  )}
/>
...
**Note**: You might have noticed that I'm using `t => t >= 0` "_greater or equal to_" in our predicate. This is just for convenience, so that we get `0` pushed onto the stream so that it gets easily rendered, finishing the countdown at `0`.
...
3. In our subscription handler, we simply do a DOM mutation to the element that displays our countdown value so that the user gets the time left displayed on the screen. Here's how this looks like conceptually.
...
Meet <a href="https://rxjs-dev.firebaseapp.com/api/operators/takeUntil" target="_blank" title="RxJS - takeUntil">takeUntil</a>, this operator allows you to stop listening to a stream until something happens! Let's look at the implementation of the `countdown$`.
...
Let's call this last stream `cancelCountdown$`. Here's the final implementation of Level 4 with cancellation. After we declare it, we can feed it as an input to our `takeUntil` operator. Here's how it looks like when putting everything together.
...
<Caption
  text={(a) => (
    <p>
      In the scenario depicted in this marbles diagram, our stream gets cancelled at after the value <b>3</b>. The first{" "}
      <b>dragend</b> event causes the <b>countdownRunning$</b> to stop emitting values.
    </p>
  )}
/>
...
<br />
<ShareHint path="/blog/hands-on-reactive-programming-rxjs" text="Hands-on Reactive Programming with RxJS" />

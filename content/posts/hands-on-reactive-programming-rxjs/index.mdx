...
- [Let's build something!](#lets-build-something)
  ...
  - [Level 2 - CHANGE TITLE Hacking race conditions (things to avoid)](#level-2---change-title-hacking-race-conditions-things-to-avoid)
...
Cool, we now have a stream that emits a `drop` event every time something is dropped in `party`. Let's see how we can detect that the `cat` is dropped at the `party`. I would like to pause and add further detail on this marble diagram. Notice that we read from top to bottom, meaning the last timeline is the only one that really matters. The **other streams in the diagram don't exist in practice**; they are just auxiliary conceptual timelines that help us understand what transformations are applied to the stream to emit its values.
...
<Caption text={() => <p>TODO: BIGGER MARBLES!!!!!!!!! TODO: name the streams...</p>} />

**What do you get if you subscribe to `catAtParty$`?**

In the above diagram, `e1`, `e2` and `e3` are `dragstart` events (`ds$` stream) that might come from anywhere in the page. `de` represents a `drop` event at the `party`. Notice how `withLatestFrom` operates by bringing together the last element on `ds$` which is `e3` and puts it together with the `party` element which is published onto the `de$` stream. Now because `e3` event target is actually the `cat` you'll get `e3` pushed onto our final stream `catAtParty$`!
...
If you're following this point, you'll notice that although we can drag the cat perfectly, we still see the original `cat` element while carrying it. To achieve the same behavior we have in the above GIF, we want to toggle the `cat` element's opacity while being dragged. Yes, another side effect, we can actually simply create another subscription to take care of this. Let's see how it works.
...
##### Level 2 - CHANGE TITLE Hacking race conditions (things to avoid)
...
<Caption
  ...
  text={() => (
    <p>
      <code>window.alert</code> is triggered when we drop the cat at the <code>party</code>, but the <code>cat</code> is
      nowhere to be seen!
    </p>
  )}
/>
...
<GIF name="take-cat-to-party" alt="take the cat to the party game demonstration" playing={false} />
<Caption gif={true} text={"It works!"} />

Works, we can achieve a similar **hack** with RxJS without having a solution based on callbacks that keep our subscription handler clean (**but still a hack!**).
...
![level3_1](./images/marbles-level1-2.png "level3_1")

<Caption text={"TODO: level3_1"} />

In the image, `ds` symbolizes a `dragstart` event, and `de` represents a `dragend` event over time.

Our GIF is an HTML element whose `display` attribute is set to `"none"'` by default. We'll want to set it to `""` (empty string) to make it visible and back to `"none"` once we decide it's time to hide it again. So basically, our side effect should look something like this:
...
Let's try to build a stream to feed to this function a value for `display` that reflects whether the cat is being dragged or not. **Our stream should remove the necessity of the imperative logic dictated by the ternary operator, we'll create values that react to changes**.
...
![level3_2](./images/marbles-level1-2.png "level3_2")

<Caption text={"TODO: level3_2"} />

Now forget the stream with the blue marbles. After you apply an operator, you transform your stream into a "new one" (not actually a new one, same stream but different output) with a more refined output-oriented to the task you're trying to tackle! The operator is represented by the white rectangle in the middle where you see `mapTo`. You're making every pushed value to the topmost timeline flow through a function provided in the `map` operator. That function will map which value into the bottom timeline, your stream. If you subscribed to it now, you would receive empty strings instead of DOM events!

The same thing should happen to `catDragEnd$`.

![level3_22](./images/marbles-level1-2.png "level3_22")

<Caption text={"TODO: missed image in the middle level3_22"} />

Here we map each event to `"none"`.

Now we have two streams, and we could just subscribe to each one of them and set the `"display"` value of the `loading` element!
...
![level3_3](./images/marbles-level1-2.png "level3_3")
...
![level4_1](./images/marbles-level1-2.png "level4_1")
...
![level4_2](./images/marbles-level1-2.png "level4_2")
...
![level4_3](./images/marbles-level1-2.png "level4_3")

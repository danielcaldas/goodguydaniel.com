...
Apart from the [last subsection](#clearing-text-from-an-input-field), what I discuss next, you can read it in the official documentation. I'm just going to step on those that I consider to be essential bits of the Puppeteer API.

##### Working with page.evaluate

You'll need to get used to the detail that when using the <a href="https://pptr.dev/#?product=Puppeteer&version=v3.0.3&show=api-pageevaluatepagefunction-args" target="_blank" title="Puppeteer Docs page.evaluate">page.evaluate API</a>,you run on the page context,
...
There's a decision you need to make. The choice is whereas you should have a higher or lower timeout. I usually try to advocate for lower as possible, because we want to keep our tests fast. Running End to End tests against systems where you need to perform (not mocked) network requests, means that you need to account for network uncertainty for test stability.
...
differently for custom select fields built with a `div > ul > li` structure and
hidden input field, for instance, things like <a href="https://material-ui.com/components/selects/#select" target="_blank" title="Select React component - Material-UI">select Material UI components</a>.
...
your test cases becomes particularly useful to get an initial pointer to what you
...
With the <a href="https://pptr.dev/#?product=Puppeteer&version=v3.1.0&show=api-pagereloadoptions" target="_blank" title="page.reload Puppeteer Documentation">page.reload</a> API. You can specify a set of options that allow you to wait for certain underlying browser tasks to idle before proceeding.
...
<Caption
  text={{
    __html:
      "E2E test on the tweak chrome extension <b>with slowMo</b>. You can see the typing in the 'URL' at a human perceptible speed",
  }}
  gif={true}
/>
...
I got this one from <a href="https://developers.google.com/web/tools/puppeteer/debugging" target="_blank" title="Debugging  |  Tools for Web Developers  |  Google Developers">Google debugging tips</a>. I had the habit of throwing a `sleep` statement to stop my tests and inspect the application to see why my tests are breaking. But now I completely shifted to this.
...
There's quite a buzz going on to use Puppeteer to automate web performance
testing. I couldn't write this article without shouting out to <a href="https://addyosmani.com/" target="_blank" title="AddyOsmani.com">Addy Osmani</a> on
the work he developed on <a href="https://github.com/addyosmani/puppeteer-webperf" target="_blank" title="Automating Web Performance testing with Puppeteer">addyosmani/puppeteer-webperf</a>, which I couldn't
